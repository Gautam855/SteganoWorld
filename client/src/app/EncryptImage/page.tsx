...
function EncryptImage() {
  ...
  const handleHideClick = async () => {
    if (
      imageInfo[0] &&
      imageInfo[1] &&
      imageInfo[1].width * imageInfo[1].height >
        imageInfo[0].width * imageInfo[0].height
    ) {
      Swal.fire({
        title: "Error",
        text: "Please Compress the Message image dimensions, Cover image should be Large",
        icon: "error",
    ...
    selectedImages.forEach((image, index) => {
      if (image) {
        formData.append(
          `image${index + 1}`,
          dataURLtoFile(image, `image${index + 1}.png`)
        );
      }
    });
    formData.append("password", password);
    const process_request = await fetch(
      "https://steganoworld-backend.onrender.com/api/encrypt_images",
      {
    ...
    <div>
      ...
      <div className="flex flex-row flex-wrap items-center justify-around">
        <ThreeDCardDemo
          ...
          handleImageChange={handleImageChange(0)}
          imageInfo={imageInfo[0]}
        />
        <ThreeDCardDemo
          index={1}
          selectedImage={selectedImages[1]}
          handleImageChange={handleImageChange(1)}
    ...
    <CardContainer className="inter-var mb-40">
      <CardBody className="bg-gray-50 relative group/card dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-auto sm:w-[30rem] h-auto rounded-xl p-6 border dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1]">
        ...
        <div className="flex justify-between items-center mt-20">
          ...
          {index === 1 &&
            handleHideClick && ( // Check if handleHideClick is defined and index is 1
              <button
